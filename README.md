...
<style>
    * { box-sizing: border-box; margin: 0; padding: 0; user-select: none; -webkit-user-select: none; -webkit-tap-highlight-color: transparent; }
    ...
    .highlight-sim { color: #30d158; font-weight: bold; }
    .highlight-cog { color: #5e5ce6; font-weight: bold; }
    ...
    button.mode-active { background: #007aff !important; color: #fff !important; font-weight: bold; border-color: #007aff !important; box-shadow: 0 0 6px rgba(0,122,255,0.4); }
    #btn-mode-angle.mode-active { background: #00ffcc !important; color: #000 !important; border-color: #00ffcc !important; }
    #btn-mode-dist.mode-active { background: #ffd60a !important; color: #000 !important; border-color: #ffd60a !important; }
    #btn-mode-acc.mode-active { background: #ff375f !important; color: #fff !important; border-color: #ff375f !important; }
    #btn-mode-sim.mode-active { background: #30d158 !important; color: #000 !important; border-color: #30d158 !important; }
    #btn-mode-cog.mode-active { background: #5e5ce6 !important; color: #fff !important; border-color: #5e5ce6 !important; }
    #btn-mode-perp.mode-active { background: #ff2d55 !important; color: #fff !important; border-color: #ff2d55 !important; }
    #btn-mode-scale.mode-active { background: #ff9500 !important; color: #000 !important; border-color: #ff9500 !important; }
    #btn-mode-ai.mode-active { background: rgba(0, 255, 204, 0.25) !important; color: #00ffcc !important; border-color: #00ffcc !important; font-weight: bold; }
...
<div id="stage">
    <div class="video-pane" id="pane1">
        <div class="zoom-target" id="zoom-target-1">
            <video id="video1" playsinline webkit-playsinline muted crossorigin="anonymous"></video>
            <canvas id="canvas1"></canvas>
        </div>
    </div>
    <div class="video-pane" id="pane2">
        <div class="zoom-target" id="zoom-target-2">
            <video id="video2" playsinline webkit-playsinline muted crossorigin="anonymous"></video>
...
<div id="controls">
    ...
    <div class="mode-grid">
        ...
        <button class="ctrl-btn" id="btn-mode-ai" onclick="setMode('ai')" style="grid-column: span 4; border: 1px solid #00ffcc; color: #00ffcc; background: rgba(0, 255, 204, 0.1);">🤖 AI骨格解析 (体幹傾斜 & 捻転差)</button>
    </div>
    ...
    function calcResults(s, mode) {
        if (mode === 'angle') {
            if (s.points.length === 3) {
                ...
                s.resultMain = s.points.length === 0 ? "角度測定: 3点を順番にタップ" : `点 [${s.points.length}/3] を記録`;
                s.resultSub = "①根元 → ②関節中心(頂点) → ③先端";
            }
        } else if (mode === 'sim') {
            if (s.points.length === 3) {
                const p1 = s.points[0], p2 = s.points[1], p3 = s.points[2];
                ...
                s.resultMain = s.points.length === 0 ? "予測モード: 根元→関節中心→先端をタップ" : `点 [${s.points.length}/3] を記録`;
            ...
            if (s.points.length === 2) {
                ...
                s.resultMain = s.points.length === 0 ? "速度測定: 始点→終点をタップ" : "始点を記録 ➔ コマ送りして終点をタップ";
                s.resultSub = "2点間の移動距離と経過時間から平均速度を算出";
            }
        } else if (mode === 'accel') {
            if (s.points.length === 3) {
                const p1 = s.points[0], p2 = s.points[1], p3 = s.points[2];
                ...
                s.resultMain = s.points.length === 0 ? "加速度測定: 連続3点をタップ" : `点 [${s.points.length}/3] を記録`;
                s.resultSub = "コマ送りしながら 始点 → 中間 → 終点 をタップ";
            }
        } else if (mode === 'perp') {
            s.resultMain = `垂直線モード`; s.resultSub = `地面などの基準2点をタップ ➔ スライダーで垂直線を移動`;
        }
    }
    ...
    function redraw() {
        [ {idx: 1, c: canvas1, ctx: ctx1, v: video1}, {idx: 2, c: canvas2, ctx: ctx2, v: video2} ].forEach(item => {
            ...
            if (s.points.length > 0 && lastMeasureMode !== 'cog') {
                ...
                else if (lastMeasureMode === 'distance') item.ctx.strokeStyle = item.ctx.fillStyle = '#ffd60a';
